#!/usr/bin/env python3
"""antivenom — remediation pack generator.

Consumes rattlesnake JSON output, performs deterministic analysis and
...
class NormalizedFinding:
    """Internal representation of a single rattlesnake finding."""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="antivenom",
        description=("Remediation pack generator for rattlesnake scan findings."),
    )
    p.add_argument(
        "--input",
        "-i",
        help="Path to rattlesnake JSON output (default: stdin)",
    )
    p.add_argument(
        "--vault",
    ...
    p.add_argument(
        "--output-dir",
        help=("Pack destination (default: ./tmp/antivenom-packs/<timestamp>/)"),
    )
    p.add_argument(
        "--combined",
        action="store_true",
...
def load_report(path: Optional[str]) -> Dict[str, Any]:
    """Read and parse rattlesnake JSON from a file or stdin."""
    if path:
        try:
        ...
        if sys.stdin.isatty():
            _fatal(
                "No input provided. Pipe rattlesnake JSON or use --input FILE."
            )
        text = sys.stdin.read()
...
def _gather_environment_lines(
    ...
    lines.append(
        f"- **Scanner:** `{SCRIPT_DIR}/rattlesnake.py`"
    )
...
def _compile_verification_tail(
    ...
    categories = sorted(set(nf.category for nf in unit.findings))
    lines.append(
        "Run rattlesnake and confirm the findings for these "
    ...
    if len(categories) == 1:
        lines.append(f"python3 rattlesnake.py --category {categories[0]} --pretty")
    else:
        lines.append("python3 rattlesnake.py --pretty")
    lines.append("```")
    if ver_paths:
        lines.append("")
...
def _render_index_entry(unit: WorkUnit) -> List[str]:
    ...
    categories = sorted(set(nf.category for nf in unit.findings))
    if len(categories) == 1:
        verify_cmd = f"python3 rattlesnake.py --category {categories[0]} --pretty"
    else:
        verify_cmd = "python3 rattlesnake.py --pretty"
...
def compile_metadata(
    ...
    lines.append(f"- **Scanner:** rattlesnake v{scanner_ver}")
    ...
    lines.extend(
        [
            ...
            "generation. A newer rattlesnake scan may "
...
def default_pack_dir() -> str:
    ...
    return os.path.join("tmp", "antivenom-packs", ts)
...
def print_pack_summary(
    ...
    print("", file=sys.stderr)
    print(bold("antivenom"), file=sys.stderr)
    print("", file=sys.stderr)
